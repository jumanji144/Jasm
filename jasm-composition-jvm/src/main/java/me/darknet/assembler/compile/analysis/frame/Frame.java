...
/**
 * Common outline of a stack-frame, agnostic of level-of-detail for the stack/locals.
 *
...
public sealed interface Frame permits TypedFrame, ValuedFrame {
	 ...
	 */
	void pushType(@Nullable Type type);

	/**
	 * @param types
	 * 		Type to push onto the stack. Can be {@code null} for {@code null} stack values.
	 */
	default void pushTypes(@Nullable Type... types) {
		for (Type type : types) {
			pushType(type);
		}
	}
